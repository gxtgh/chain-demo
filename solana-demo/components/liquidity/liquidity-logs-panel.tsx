import type { BatchTaskLog } from "../../types/task";
import styles from "./liquidity-shell.module.scss";

export function LiquidityLogsPanel(props: { logs: BatchTaskLog[] }) {
  return (
    <article className={styles.panel}>
      <h2 className={styles.panelTitle}>流动性日志</h2>
      <p className={styles.panelDescription}>记录当前页面的参数建模、适配器输出和后续接协议的提示。</p>
      <div className={styles.panelBody}>
        {props.logs.length > 0 ? (
          <div className={styles.logList}>
            {props.logs.map((log, index) => (
              <div key={`${log.timestamp}-${index}`} className={styles.logItem}>
                <div className={styles.logMeta}>
                  {log.level} · {log.timestamp ?? "unknown time"}
                </div>
                <div className={styles.logMessage}>{log.message}</div>
              </div>
            ))}
          </div>
        ) : (
          <div className={styles.empty}>当前页面还没有日志。</div>
        )}
      </div>
    </article>
  );
}

