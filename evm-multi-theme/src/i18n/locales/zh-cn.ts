...
export const zhCnMessages = {
  ...
  tokenDividendCreation: {
    ...
    title: '创建分红代币',
    description:
      '在 {{chain}} 上创建 {{tokenType}} 分红代币，支持同币分红与外部代币分红模式，并可同时配置税率、阈值、名单和手动开盘参数。',
    seo: {
      title: '创建 {{chain}} {{tokenType}} 分红代币 | 同币与外部代币分红',
      description:
        '快速创建 {{chain}} {{tokenType}} 分红代币，支持同币分红、外部代币分红、买卖转税拆分、名单配置和开盘控制。',
      keywords: '创建分红代币, {{chain}} 分红代币创建, {{tokenType}} Dividend Token, 同币分红, 外部代币分红',
    },
    fields: {
      name: '代币全称',
      symbol: '代币简称',
      ...
      poolToken: '底池配对币',
      dividendToken: '分红奖励币',
      isSameTokenDividend: '同币分红模式',
      receiveAddress: '代币接收地址',
      fundAddress: '基金地址',
      minHoldingForDividend: '最小分红持仓',
      dividendTriggerThreshold: '触发分红阈值',
      autoProcessGasLimit: '自动处理 Gas',
      killBlockCount: '杀区块数量',
      mintEnabled: '开启增发权限',
      manualTradingEnable: '手动开盘',
      whitelistEnabled: '启用白名单',
      blacklistEnabled: '启用黑名单',
      initialWhitelist: '初始白名单地址',
      initialBlacklist: '初始黑名单地址',
    },
    labels: {
      modeTitle: '分红模式',
      modeDescription: '先明确分红资产和交易所路由，再继续填写阈值、税率和权限参数。',
      sameTokenModeHelp: '同币分红模式下，税费中的分红部分会按代币自身资产进入分红流程。',
      externalTokenModeHelp: '外部代币分红模式下，税费中的分红部分会先通过路由换成奖励币，再进行分发。',
      distributionRules: '分红参数',
      distributionRulesNote: '这组参数决定代币初始接收地址、基金地址、最小参与门槛和自动处理强度。',
      taxConfiguration: '税率矩阵',
      taxConfigurationNote: '税率按买入、卖出、转账三组分别填写，每组再拆分为基金、回流、销毁和分红四段。',
      totalTaxRate: '三组税率总和',
      permissionControls: '权限与名单',
      permissionControlsNote: '这里定义是否允许增发、是否采用手动开盘，以及是否启用黑白名单。',
      creationFee: '创建费',
    },
    modes: {
      sameToken: '同币分红',
      externalToken: '外部代币分红',
    },
    taxGroups: {
      buy: '买入税率',
      sell: '卖出税率',
    ...
    taxTooltips: {
      MarketingTax: '这部分税费会进入基金地址。',
      ReflowTax: '这部分税费用于流动性相关处理或回流逻辑。',
      BurnTax: '这部分税费会在合约逻辑中转入销毁方向。',
      DividendTax: '这部分税费会进入分红处理流程。',
    },
    placeholders: {
      name: '请输入代币全称',
      symbol: '请输入代币简称',
      totalSupply: '请输入发行量',
      decimals: '请输入精度',
      exchange: '请选择支持的工厂和交易所',
      poolToken: '请选择或输入底池币地址',
      dividendToken: '请选择或输入奖励币地址',
      receiveAddress: '留空默认当前钱包地址',
      fundAddress: '留空默认当前钱包地址',
      minHoldingForDividend: '请输入最小分红持仓',
      dividendTriggerThreshold: '请输入触发分红阈值',
      autoProcessGasLimit: '请输入自动处理 Gas',
      killBlockCount: '请输入杀区块数量，默认 0',
    ...
    tooltips: {
      ...
      exchange: '选择要使用的分红代币工厂和对应路由，合约部署时会写入该路由地址。',
      poolToken: '用于建立主交易对的底池币，若选择原生币，部署时会自动使用对应 Wrapped Token。',
      dividendToken: '外部代币分红模式下的奖励资产，若选择原生币，部署时会自动使用 Wrapped Token。',
      isSameTokenDividend: '开启后表示同币分红；关闭后需要额外指定奖励币地址。',
      receiveAddress: '代币初始接收地址；留空时默认使用当前连接钱包地址。',
      fundAddress: '基金地址；留空时默认使用当前连接钱包地址。',
      minHoldingForDividend: '只有持仓达到该门槛的地址才会参与分红。',
      dividendTriggerThreshold: '累计到该阈值后，合约才会尝试处理分红流程。',
      autoProcessGasLimit: '自动分红处理时允许使用的 Gas 上限。',
      killBlockCount: '手动开盘后前若干区块可执行特殊防护逻辑。',
      mintEnabled: '开启后 Owner 可在后续控制台中继续增发。',
      manualTradingEnable: '开启后需要手动开盘；关闭则部署后交易逻辑直接开放。',
      whitelistEnabled: '开启后合约会启用白名单机制。',
      blacklistEnabled: '开启后合约会启用黑名单机制。',
    ...
    errors: {
      ...
      fundAddressInvalid: '请输入有效的基金地址。',
      exchangeRequired: '请选择可用的分红工厂。',
      poolTokenRequired: '请选择或输入底池币。',
      dividendTokenRequired: '外部代币分红模式下必须指定奖励币。',
      dividendTokenInvalid: '请输入有效的奖励币地址。',
      minHoldingInvalid: '请输入大于 0 的最小分红持仓。',
      dividendTriggerThresholdInvalid: '请输入大于 0 的分红触发阈值。',
      autoProcessGasLimitInvalid: '自动处理 Gas 必须是大于 0 的整数。',
      ...
      fundAddressRequired: '基金地址不能为空。',
      fundAddressDead: '基金地址不能设置为销毁地址。',
    },
    success: {
      banner: '分红代币创建成功',
      tokenAddress: '代币地址',
    ...
    overview: {
      title: '分红代币模型概览',
      description: '这套 {{tokenType}} 分红代币模型围绕“创建即具备后续维护入口”来设计，支持多种分红资产模式和更细的运营参数控制。',
      highlights: {
        mode: '双分红模式',
        tax: '三段税率矩阵',
        reward: '奖励资产可切换',
      ...
      cards: {
        dividendModes: {
          title: '分红模式',
          description: '支持同币分红和外部代币分红，两种模式共用同一套创建结构和后续控制逻辑。',
        },
        taxFlow: {
          title: '税率流向',
          description: '买入、卖出、转账三组税率都可拆分到基金、回流、销毁和分红四个方向。',
        },
        management: {
          title: '后续维护',
          description: '部署完成后可继续进入控制台执行名单管理、参数更新、手动分红和权限处理。',
        },
      },
    },
    nextSteps: {
      title: '下一步建议',
      description: '分红代币创建完成后，建议继续完成以下动作，确保链上状态和后续运营流程都清晰可控。',
      importWallet: {
        title: '导入钱包与浏览器',
        description: '确认代币地址、简称和精度展示正常，便于内部联调和外部展示。',
      },
      addLiquidity: {
        title: '建立交易对并加池',
        description: '按创建时选择的底池币在目标 DEX 建立主交易对并注入初始流动性。',
      },
      openConsole: {
        title: '进入控制台',
        description: '创建成功后进入控制台继续维护税率、分红处理、名单和权限状态。',
      },
      verifyRoute: {
        title: '验证分红路径',
        description: '尤其是外部代币分红模式，需要确认奖励币换币路径和分红逻辑都能按预期执行。',
      },
      note: '不同链、DEX 和钱包的展示与同步时间可能存在差异，请以链上状态和实际联调结果为准。',
    },
    modal: {
